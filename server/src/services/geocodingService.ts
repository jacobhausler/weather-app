 ...
 */
async function fetchFromZippopotamAPI(zipCode: string): Promise<Coordinates> {
  try {
    ...
    if (!places || places.length === 0) {
      throw createError(
        'NO_RESULTS',
        `No location data found for ZIP code: ${zipCode}`
      );
    }
    ...
    if (!firstPlace) {
      throw createError(
        'NO_RESULTS',
        `No valid location found for ZIP code: ${zipCode}`
      );
    }
    ...
    if (
      ...
      throw createError(
        'API_ERROR',
        `Invalid coordinates returned for ZIP code: ${zipCode} (lat: ${lat}, lon: ${lon})`
      );
    }
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (axiosError.code === 'ECONNABORTED' || axiosError.code === 'ETIMEDOUT') {
        throw createError(
          'NETWORK_ERROR',
          'Zippopotam API request timed out',
          error
        );
      }
      ...
      if (!axiosError.response) {
        throw createError(
          'NETWORK_ERROR',
          'Unable to reach Zippopotam API',
          error
        );
      }
      ...
      throw createError(
        'API_ERROR',
        `Zippopotam API returned status ${axiosError.response.status}`,
        error
      );
    }
 ...
 * ```
 */
export async function geocodeZip(zipCode: string): Promise<Coordinates> {
  // Normalize input
  ...
  if (!isValidZipCode(normalizedZip)) {
    throw createError(
      'INVALID_ZIP',
      `Invalid ZIP code format. Must be 5 digits. Received: ${zipCode}`
    );
  }

  // Check cache first
  const cached = geocodeCache.get<Coordinates>(normalizedZip);
  if (cached) {
    return cached;
  }
