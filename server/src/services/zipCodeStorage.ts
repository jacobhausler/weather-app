...
let zipCodes: Set<string> = new Set();

// Track initialization state
let initialized = false;

/**
 * ZIP code storage data structure
 */
 ...
 */
export async function initZipCodeStorage(): Promise<void> {
  if (initialized) {
  ...
  try {
    ...
    try {
      ...
      const data: ZipCodeData = JSON.parse(fileContent);

      zipCodes = new Set(data.zipCodes);
      logger.info(
        `[ZipStorage] Loaded ${zipCodes.size} ZIP codes from storage file`,
      ...
      if (envZipCodes) {
        ...
        logger.info(
          `[ZipStorage] Initialized with ${zipCodes.size} ZIP codes from environment`,
          { zipCodes: Array.from(zipCodes) }
        );
    ...
    if (envZipCodes) {
      ...
      zipCodes = new Set(initialZips);
      logger.warn(
        `[ZipStorage] Using in-memory storage with ${zipCodes.size} ZIP codes from environment`
      );
    }
 ...
 */
async function saveZipCodes(): Promise<void> {
  try {
 ...
 */
export async function addZipCode(zipCode: string): Promise<void> {
 ...
 */
export async function removeZipCode(zipCode: string): Promise<boolean> {
