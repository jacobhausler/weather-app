...
export class NWSService {
    ...
    cache;
    userAgent;
    constructor(userAgent = 'WeatherApp/1.0 (contact@example.com)') {
        ...
        this.client.interceptors.response.use((response) => response, (error) => {
            if (error.response?.data) {
                ...
                throw new Error(`NWS API Error: ${nwsError.title} - ${nwsError.detail || 'No additional details'}`);
            }
            throw error;
        });
    }
    ...
    async makeRequest(url, retries = MAX_RETRIES) {
        for (let attempt = 0; attempt < retries; attempt++) {
            try {
                ...
                if (attempt === retries - 1) {
                    throw new Error(`Failed to fetch data from NWS API: ${axiosError.message}`);
                }
     ...
     */
    async getCachedOrFetch(cacheKey, ttl, fetchFn) {
        // Check cache first
     ...
     */
    async getStations(gridId, gridX, gridY) {
     ...
     */
    async getCurrentConditions(gridId, gridX, gridY) {
        try {
...
//# sourceMappingURL=nwsService.js.map