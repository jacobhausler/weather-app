...
const weatherRoutes = async (fastify) => {
    ...
    fastify.get('/weather/:zipcode', {
        ...
        try {
            ...
            try {
                ...
                fastify.log.debug({ zipcode, coordinates }, 'Successfully geocoded ZIP code');
            }
            catch (error) {
            ...
            try {
                ...
                const errorMessage = error instanceof Error ? error.message : 'Failed to get point data';
            ...
            const { gridId, gridX, gridY, forecastOffice, timeZone } = pointData.properties;
            // Step 4: Fetch all weather data in parallel
            // Use Promise.allSettled to handle partial failures gracefully
            const [forecastResult, hourlyForecastResult, currentConditionsResult, alertsResult, uvIndexResult,] = await Promise.allSettled([
            ...
            if (forecastResult.status === 'rejected') {
                fastify.log.error({ error: forecastResult.reason, zipcode }, 'Failed to fetch forecast data');
            ...
            if (hourlyForecastResult.status === 'rejected') {
                fastify.log.error({ error: hourlyForecastResult.reason, zipcode }, 'Failed to fetch hourly forecast data');
                return reply.code(500).send({
                    error: 'Internal Server Error',
            ...
            if (currentConditionsResult.status === 'rejected') {
                fastify.log.warn({ error: currentConditionsResult.reason, zipcode }, 'Failed to fetch current conditions');
            }
            if (alertsResult.status === 'rejected') {
                fastify.log.warn({ error: alertsResult.reason, zipcode }, 'Failed to fetch alerts');
            }
            if (uvIndexResult.status === 'rejected') {
                fastify.log.warn({ error: uvIndexResult.reason, zipcode }, 'Failed to fetch UV Index');
            }
            ...
            const weatherPackage = {
                ...
                currentConditions: currentConditionsResult.status === 'fulfilled'
                ...
                sunTimes,
                uvIndex: uvIndexResult.status === 'fulfilled' ? uvIndexResult.value : null,
                metadata: {
            ...
            fastify.log.error({ error, zipcode, errorMessage }, 'Unexpected error fetching weather data');
            return reply.code(500).send({
                error: 'Internal Server Error',
     ...
     */
    fastify.post('/weather/cache/clear/:zipcode', async (request, reply) => {
        const { zipcode } = request.params;
        try {
     ...
     */
    fastify.post('/weather/:zipcode/refresh', {
        schema: {
            params: {
                type: 'object',
                required: ['zipcode'],
        ...
        try {
            ...
            try {
                coordinates = await geocodeZip(zipcode);
                fastify.log.debug({ zipcode, coordinates }, 'Successfully geocoded ZIP code for refresh');
            }
            catch (error) {
                const errorMessage = error instanceof Error ? error.message : 'Geocoding failed';
                if (errorMessage.includes('No geocoding results found')) {
            ...
            if (forecastResult.status === 'rejected') {
                fastify.log.error({ error: forecastResult.reason, zipcode }, 'Failed to fetch forecast data during refresh');
                return reply.code(500).send({
                    error: 'Internal Server Error',
                    message: 'Failed to fetch forecast data. Please try again later.',
                    statusCode: 500,
            ...
            if (hourlyForecastResult.status === 'rejected') {
                fastify.log.error({ error: hourlyForecastResult.reason, zipcode }, 'Failed to fetch hourly forecast data during refresh');
                return reply.code(500).send({
                    error: 'Internal Server Error',
                    message: 'Failed to fetch hourly forecast data. Please try again later.',
                    statusCode: 500,
            ...
            // Log warnings for non-critical failures
            if (currentConditionsResult.status === 'rejected') {
                fastify.log.warn({ error: currentConditionsResult.reason, zipcode }, 'Failed to fetch current conditions during refresh');
            }
            if (alertsResult.status === 'rejected') {
                fastify.log.warn({ error: alertsResult.reason, zipcode }, 'Failed to fetch alerts during refresh');
            }
            if (uvIndexResult.status === 'rejected') {
                fastify.log.warn({ error: uvIndexResult.reason, zipcode }, 'Failed to fetch UV Index during refresh');
            }
            ...
            // Catch-all error handler
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
            fastify.log.error({ error, zipcode, errorMessage }, 'Unexpected error refreshing weather data');
            return reply.code(500).send({
                error: 'Internal Server Error',
                message: 'An unexpected error occurred while refreshing weather data. Please try again later.',
                statusCode: 500,
            });
        }
    });
